...
### 👤 人性化设计
- **域名主体管理员**：自动提取域名主体作为管理员用户名（如 `deepseek.com` → 账号：`deepseek`），安全且易记
...
```bash
# 使用 curl
bash <(curl -s https://raw.githubusercontent.com/你的用户名/1kwp-debian/main/1kwp-debian.sh) 你的邮箱 你的主域名 [其他域名...]

# 使用 wget
bash <(wget -qO- https://raw.githubusercontent.com/你的用户名/1kwp-debian/main/1kwp-debian.sh) 你的邮箱 你的主域名 [其他域名...]
```

### 使用示例
```bash
# 单域名安装（管理员账号：example）
bash <(curl -s https://raw.githubusercontent.com/你的用户名/1kwp-debian/main/1kwp-debian.sh) admin@example.com example.com

# 多域名安装（绑定主域名和 www）
bash <(curl -s https://raw.githubusercontent.com/你的用户名/1kwp-debian/main/1kwp-debian.sh) admin@example.com example.com www.example.com

# 复杂域名安装（绑定多个子域名）
bash <(curl -s https://raw.githubusercontent.com/你的用户名/1kwp-debian/main/1kwp-debian.sh) admin@company.com company.com www.company.com shop.company.com blog.company.com
```
...
### Q: 管理员账号为什么是域名主体？
**A**: 这是脚本的安全易用设计：`deepseek.com` → 账号 `deepseek`。相比默认的 `admin` 更安全（避免广谱攻击），又比完全随机的用户名更容易记忆。
