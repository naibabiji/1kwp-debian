...
get_system_resources() {
    ...
    # 检查磁盘空间
    if [ "$(echo "$AVAILABLE_SPACE_GB < 7" | bc 2>/dev/null || echo 1)" -eq 1 ]; then
        log_error "可用磁盘空间不足7GB"
        echo "当前可用: ${AVAILABLE_SPACE_GB}GB"
        echo "安装需要约6GB空间，请清理磁盘空间或扩容后再运行脚本"
        return 1
    elif [ "$(echo "$AVAILABLE_SPACE_GB < 10" | bc 2>/dev/null || echo 0)" -eq 1 ]; then
...
main() {
    ...
    echo ""
    echo "📝 特点:"
    echo "  • 使用域名主体作为管理员账号 (如 deepseek.com → 账号: deepseek)"
    ...
    if [ $# -lt 2 ]; then
        ...
        echo ""
        echo "📋 示例:"
        echo "  $0 user@example.com example.com"
        echo "  $0 user@example.com example.com www.example.com"
        echo "  $0 user@example.com deepseek.com www.deepseek.com"
        echo ""
        echo "📝 说明:"
        echo "  1. 第一个参数必须是邮箱地址（用于SSL证书通知）"
        echo "  2. 后续参数为需要绑定的域名，至少一个，支持多个"
        echo "  3. 主域名的域名主体将作为管理员账号 (如 deepseek.com → 账号: deepseek)"
